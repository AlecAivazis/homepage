...
a schema that represents our domain. This model should represent your product's domain regardless of the
persistence and execution details. Ideally, a centralized and decentralized version of the same application
would have almost identical schemas. Whereas before the schema acted as an agreement between the server and client,
...
comes from sources both on and off the chain. Since we have a schema from the standard libraries, we can
