...
_This post is not meant to be your first introduction to ethereum or graphql.
...
stitched with schemas from remote APIs for powerful hybrid applications without complicating the user
interface with those details.
...
the state of our contracts. To keep things focused, we only went over how to resolve a query against the
blockchain. However, there is a lot of things that can be added on top to make the developer experience
...
-   Tying [contract events](https://web3js.readthedocs.io/en/1.0/web3-eth-contract.html#events) into
    GraphQL subscriptions for real time user interfaces.
